...
class HomeTimelineCollectionModel extends CollectionModel {
  ...
  FutureOr<IDataCollection> build(ModelContext context) async {
    ...
    return await FirestoreCollection.listen("$target?homeFilteredBy=$userId",
      ...
      ..joinWhere(
        ...
        builder: (collection) {
          return FirestoreCollection.listen("user/$userId/like?${target}Joined",
              query: collection.length <= 0
                  ? FirestoreQuery.empty()
                  : FirestoreQuery.inArray(
                      "uid", collection.map((e) => e.uid)));
        },
